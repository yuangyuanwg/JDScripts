...
main();
async function main() {
  $.http.get({url: `https://purge.jsdelivr.net/gh/lxk0301/jd_scripts@master/jd_apple_live.js`}).then((resp) => {
  ...
  if ($.body) {
    $.body = $.body.replace(
      /(const inviteCodes = \[)[^\]]+/,
      "$1'P04z54XCjVUm4aW5khRQiW7'"
    );
    eval($.body);
  }
}
function updateShareCodes(url = 'https://raw.githubusercontent.com/lxk0301/jd_scripts/master/jd_apple_live.js') {
...
function updateShareCodesCDN(url = 'https://cdn.jsdelivr.net/gh/lxk0301/jd_scripts@master/jd_apple_live.js') {
  return new Promise(resolve => {
    $.get({url}, async (err, resp, data) => {
      try {
        if (err) {
