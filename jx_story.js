...
main();
async function main() {
  $.http.get({url: `https://purge.jsdelivr.net/gh/lxk0301/jd_scripts@master/jd_jxstory.js`}).then((resp) => {
  ...
  if ($.body) {
    $.body = $.body.replace(
      /(const inviteCodes = \[)[^\]]+/,
      "$1'cPtBYMgOnW-7L5504uVY2A=='"
    );
    eval($.body);
  }
}
function updateShareCodes(url = 'https://raw.githubusercontent.com/lxk0301/jd_scripts/master/jd_jxstory.js') {
...
function updateShareCodesCDN(url = 'https://cdn.jsdelivr.net/gh/lxk0301/jd_scripts@master/jd_jxstory.js') {
  return new Promise(resolve => {
    $.get({url}, async (err, resp, data) => {
      try {
        if (err) {
