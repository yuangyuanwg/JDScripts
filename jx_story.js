...
async function main() {
  ...
  if ($.body) {
    $.body = $.body.replace(
      ...
      'const _name = $.info && $.info.shareId && $.info.shareId.slice(0,8);await submitInviteId(_name ? "jx_" + _name : userName);'
    ).replace(
      '// const jdCookieNode',
      'const jdCookieNode'
    );
...
function updateShareCodesCDN(url = 'https://cdn.jsdelivr.net/gh/whyour/hundun@master/quanx/jx_story.js') {
  return new Promise(resolve => {
    $.get({url}, async (err, resp, data) => {
      try {
        if (err) {
