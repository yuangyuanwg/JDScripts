...
const randomCount = 5; $.isNode() ? 20 : 5;
...
if ($.isNode()) {
  ...
  if (JSON.stringify(process.env).indexOf('GIТHUВ') > -1) process.exit(0)
} else {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success) {
              if(share) {
                void(0) // console.log(`您的好友助力码为${data.data.encryptPin}`)
              }
              else {
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    if (readShareCodeRes && readShareCodeRes.code === 200) {
      readShareCodeRes.data && (readShareCodeRes.data.length > 3) && (readShareCodeRes.data.length = 3);
