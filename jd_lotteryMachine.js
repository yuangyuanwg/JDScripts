  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    for (let oneAppId in $.allShareId) {
      ...
      for (let j = 0; j < oneAcHelpList.length; j++) {
        ...
        void(0) // console.log(`账号${i + 1} ${$.UserName} 去助力账号 ${$.item['userName']}的第${$.item['index']}个抽奖活动【${$.item['appId']}】，邀请码 【${$.item['taskToken']}】`)
        $.canHelp = true;
            ...
            if (data.data && data.data.bizCode === 0) {
              for (let item of data.data.result.taskVos) {
                if ([14, 6].includes(item.taskType)) {
                  void(0) // console.log(`邀请码：${item.assistTaskDetailVo.taskToken}`)
